...
const WatchlistState = ({ children }) => {
	...
	const deleteWatchlist = async (watchlist_id) => {
		...
		try {
			const res = await axios.delete(
				`${baseApi}/api/watchlist/delete/${watchlist_id}`,
				config
			);
			if (res.status === 200) {
	...
	const getAllWatchlists = useCallback(async () => {
		...
		try {
			const res = await axios.get(
				`${baseApi}/api/watchlist/allwatchlists`,
				config
			);
			dispatch({
	...
	const getWatchlist = useCallback(async (watchlist_id) => {
		...
		try {
			const res = await axios.get(
				`${baseApi}/api/watchlist/get/${watchlist_id}`,
				config
			);
			dispatch({
				type: GET_WATCHLIST,
	...
	const addTitle = async (watchlist_id, title) => {
		...
		try {
			const res = await axios.post(
				`${baseApi}/api/watchlist/add/${watchlist_id}`,
				body,
				config
			);
			if (res.status === 200) {
	...
	const removeTitle = async (watchlist_id, title_id) => {
		...
		try {
			const res = await axios.delete(
				`${baseApi}/api/watchlist/remove/${watchlist_id}/${title_id}`,
				config
			);
			if (res.status === 200) {
				dispatch({
