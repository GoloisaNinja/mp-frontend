...
const ProfileState = ({ children }) => {
	...
	const baseApi = process.env.REACT_APP_MPAPI_BASEURL;
	...
	const getProfile = async () => {
		...
		try {
			...
			const res = await axios.get(`${baseApi}/api/profile/me`, config);
			dispatch({
				type: GET_PROFILE,
				payload: res.data,
			});
	...
	const getAllProfiles = useCallback(async () => {
		...
		try {
			...
			const res = await axios.get(`${baseApi}/api/profile/all`, config);
			dispatch({
	...
	const inviteWatchlist = async (watchlist_id, user_id, watchlist_name) => {
		...
		try {
			const res = await axios.post(
				`${baseApi}/api/watchlist/invite/${watchlist_id}/${user_id}`,
				body,
				config
			);
			if (res.status === 201) {
	...
	const declineInvite = async (invite_id) => {
		...
		try {
			const res = await axios.post(
				`${baseApi}/api/watchlist/decline/${invite_id}`,
				config
			);
			if (res.status === 200) {
				dispatch({
	...
	const acceptInvite = async (invite_id, watchlist_id) => {
		...
		try {
			const res = await axios.post(
				`${baseApi}/api/watchlist/accept/${invite_id}/${watchlist_id}`,
				config
			);
			if (res.status === 200) {
				dispatch({
	...
	const createProfile = async (formData) => {
		...
		const body = formData;
		try {
			const res = await axios.post(`${baseApi}/api/profile`, body, config);
			dispatch({
				type: CREATE_PROFILE,
				payload: res.data,
			});
