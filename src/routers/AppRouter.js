...
const AppRouter = () => {
	...
	const registerUser = async (name, email, password) => {
		try {
			...
			const res = await axios.post(`${baseApi}/api/user`, body, config);
			if (res.status === 201) {
				dispatch({
	...
	const loginUser = async (email, password) => {
		try {
			...
			const res = await axios.post(`${baseApi}/api/user/login`, body, config);
			if (res.status === 200) {
				dispatch({
					type: LOGIN_USER,
					payload: res.data,
	...
	const logoutUser = async (history) => {
		...
		try {
			const res = await axios.post(`${baseApi}/api/user/logout`, config);
			if (res.status === 200) {
				dispatch({
					type: LOGOUT_USER,
				});
	...
	const loadUser = async () => {
		...
		try {
			const res = await axios.get(`${baseApi}/api/user/auth`);
			if (res.status === 200) {
				dispatch({
					type: LOAD_USER,
					payload: res.data,
